...
class AIWrapper:
    ...
    def _ai_matrix_multiply(self, A, B):
        assert A.shape[0]==B.shape[1] and \
            self.is_power_of_x(A.shape[0], 4) and \
            self.is_power_of_x(A.shape[1], 5) and \
            self.is_power_of_x(B.shape[0], 5) and \
            self.is_power_of_x(B.shape[1], 5), "Incorrect matrix sizes, must be powers of 4x5 and 5x5"
